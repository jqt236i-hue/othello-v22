    ...
    if (result.converted.length > 0) {
        ...
        for (const pos of result.converted) {
            ...
            if (typeof emitPresentationEvent === 'function') {
                emitPresentationEvent(cardState, { type: 'CROSSFADE_STONE', row: pos.row, col: pos.col, effectKey: 'regenStone', owner: player, fadeIn: false, durationMs: 600 });
            }
        }
        ...
        if (regenRes.regened && regenRes.regened.length) {
            ...
            for (const pos of regenRes.regened) {
                ...
                if (typeof emitPresentationEvent === 'function') {
                    emitPresentationEvent(cardState, { type: 'CROSSFADE_STONE', row: pos.row, col: pos.col, effectKey: 'regenStone', owner: ownerColor, newColor: ownerColor, durationMs: 600, autoFadeOut: true, fadeWholeStone: true });
                }
            }
        }
    ...
    if (result.converted.length > 0) {
        ...
        if (regenRes.regened && regenRes.regened.length) {
            ...
            for (const pos of regenRes.regened) {
                ...
                if (typeof emitPresentationEvent === 'function') {
                    emitPresentationEvent(cardState, { type: 'CROSSFADE_STONE', row: pos.row, col: pos.col, effectKey: 'regenStone', owner: actualOwnerColor, newColor: actualOwnerColor, durationMs: 600, autoFadeOut: true, fadeWholeStone: true });
                }
            }
        }
