...
(function (root, factory) {
    ...
    function applyTurnStartPhase(CardLogic, Core, cardState, gameState, playerKey, events, prng) {
        ...
        if (cardState.lastTurnStartedFor !== playerKey) {
            ...
            for (const b of (cardState.bombs || [])) {
                markers.push({ kind: 'bomb', bomb: b, createdSeq: (b.createdSeq || 0) });
            }
            for (const s of (cardState.specialStones || [])) {
                markers.push({ kind: 'special', stone: s, type: s.type, owner: s.owner, row: s.row, col: s.col, createdSeq: (s.createdSeq || 0) });
            }
            markers.sort((a, b) => (a.createdSeq || 0) - (b.createdSeq || 0));

            const hyperAggregated = { moved: [], destroyed: [], flipped: [], flippedByOwner: { black: [], white: [] } };

            for (const m of markers) {
                if (m.kind === 'bomb') {
                    const res = CardLogic.tickBombAt(cardState, gameState, m.bomb, playerKey);
                    if (res && res.exploded && res.exploded.length) {
                        events.push({ type: 'bombs_exploded', details: res });
                    }
                } else if (m.kind === 'special') {
                    const t = (m.type || '').toUpperCase();
                    if (t === 'ULTIMATE_DESTROY_GOD' && m.owner === playerKey) {
                        const res = CardLogic.processUltimateDestroyGodEffectsAtTurnStartAnchor(cardState, gameState, playerKey, m.row, m.col);
                    ...
                    } else if (t === 'DRAGON' && m.owner === playerKey) {
                        const res = CardLogic.processDragonEffectsAtTurnStartAnchor(cardState, gameState, playerKey, m.row, m.col);
                    ...
                    } else if (t === 'BREEDING' && m.owner === playerKey) {
                        const res = CardLogic.processBreedingEffectsAtTurnStartAnchor(cardState, gameState, playerKey, m.row, m.col, p);
                        ...
                        const ownerKey = m.owner;
                        if (typeof console !== 'undefined' && console.log) console.log('[TurnPipeline] processing HYPERACTIVE anchor', { row: m.row, col: m.col, owner: ownerKey, createdSeq: m.createdSeq });
                        const res = CardLogic.processHyperactiveMoveAtAnchor(cardState, gameState, ownerKey, m.row, m.col, p);
                        if (typeof console !== 'undefined' && console.log) console.log('[TurnPipeline] hyperactive result', { row: m.row, col: m.col, owner: ownerKey, res });
    ...
    function applyCardUsagePhase(CardLogic, cardState, gameState, playerKey, action, events) {
        if (action.useCardId) {
            const ok = CardLogic.applyCardUsage(cardState, gameState, playerKey, action.useCardId);
            if (!ok) {
    ...
    function applyActionPhase(CardLogic, Core, cardState, gameState, playerKey, action, events, prng, BoardOps) {
        ...
        if (action.type === 'pass') {
            ...
            // GOLD/SILVER: the placed stone disappears after use (spec behavior)
            if (effects && (effects.goldStoneUsed || effects.silverStoneUsed)) {
                if (BoardOps && typeof BoardOps.destroyAt === 'function') {
                    BoardOps.destroyAt(cardState, gameState, action.row, action.col, 'SYSTEM', 'multiplier_stone_disappeared');
                } else {
                    gameState.board[action.row][action.col] = Core.EMPTY;
                }
                events.push({ type: 'multiplier_stone_disappeared', row: action.row, col: action.col });
            }
