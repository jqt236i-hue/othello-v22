...
(function (root, factory) {
    ...
    function clearBombAt(cardState, row, col) {
        if (!cardState || !cardState.bombs || !cardState.bombs.length) return false;
        const beforeLen = cardState.bombs.length;
        cardState.bombs = cardState.bombs.filter(b => !(b.row === row && b.col === col));
        return cardState.bombs.length !== beforeLen;
    }
    ...
    function createCardState(prng) {
        ...
        return {
            ...
            specialStones: [],        // [{ row, col, type, owner, expiresForPlayer?, remainingOwnerTurns?, autoRemove? }]
            bombs: [],                // [{ row, col, remainingTurns, owner }] - separate for area destruction
    ...
    function copyCardState(cs) {
        return {
            ...
            specialStones: cs.specialStones.map(s => ({ ...s })),
            bombs: cs.bombs.map(b => ({ ...b })),
    ...
    function addMarker(cardState, kind, row, col, owner, data) {
        ...
        // Sync to legacy arrays for backward compatibility and include createdSeq there as well
        if (kind === 'specialStone') {
            // Diagnostic: log when special stones are added, especially WORK markers
            try {
                if (data && data.type === 'WORK') console.log('[WORK_DEBUG] addMarker: adding WORK specialStone at', row, col, 'owner:', owner, 'data:', data);
            } catch (e) {}
    ...
    function removeMarkerById(cardState, markerId) {
        ...
        if (marker.kind === 'specialStone') {
            const legacyIndex = cardState.specialStones.findIndex(
                s => s.row === marker.row && s.col === marker.col && s.type === marker.data.type
            );
            if (legacyIndex !== -1) {
                cardState.specialStones.splice(legacyIndex, 1);
            }
        } else if (marker.kind === 'bomb') {
            const legacyIndex = cardState.bombs.findIndex(
                b => b.row === marker.row && b.col === marker.col
            );
            if (legacyIndex !== -1) {
                cardState.bombs.splice(legacyIndex, 1);
            }
        }
    ...
    function applyTemptWill(cardState, gameState, playerKey, row, col) {
        ...
        if (cardState.specialStones && cardState.specialStones.length) {
            const prev = cardState.specialStones.find(s => s.row === row && s.col === col);
            wasWork = !!(prev && prev.type === 'WORK');
            cardState.specialStones = cardState.specialStones.map(s => {
                if (s.row !== row || s.col !== col) return s;
                const next = { ...s, owner: playerKey };
                if (next.expiresForPlayer !== undefined) next.expiresForPlayer = playerKey;
                return next;
            });
        }
        if (cardState.bombs && cardState.bombs.length) {
            cardState.bombs = cardState.bombs.map(b => {
                if (b.row !== row || b.col !== col) return b;
                return { ...b, owner: playerKey };
            });
        }
        ...
        if (wasWork) {
            ...
            cardState.specialStones = (cardState.specialStones || []).filter(s => !(s.row === row && s.col === col && s.type === 'WORK'));
            cardState.markers = (cardState.markers || []).filter(m => !(m.kind === 'specialStone' && m.data && m.data.type === 'WORK' && m.row === row && m.col === col));
    ...
    function onTurnStart(cardState, playerKey, gameState, prng) {
        ...
        if (cardState.turnCountByPlayer[playerKey] % DRAW_INTERVAL === 0) {
        ...
        cardState.specialStones = cardState.specialStones.filter(s => {
            // expiresForPlayer check (e.g., PROTECTED)
            if (s.expiresForPlayer === playerKey) return false;
            if (s.remainingOwnerTurns !== undefined && s.remainingOwnerTurns <= 0) return false;
            if (s.type === 'REGEN' && (s.regenRemaining || 0) <= 0) return false;
    ...
    function applyStrongWill(cardState, playerKey, row, col) {
        ...
        const already = cardState.specialStones.some(s =>
            s.row === row && s.col === col && s.type === 'PERMA_PROTECTED'
        );
        if (!already) {
            addMarker(cardState, 'specialStone', row, col, playerKey, {
    ...
    */
    ...
    function tickBombAt(cardState, gameState, bomb, activeKey) {
        ...
        const bombs = cardState.bombs || [];
        const idx = bombs.findIndex(b => b.row === bomb.row && b.col === bomb.col && b.owner === bomb.owner && b.createdSeq === bomb.createdSeq);
    ...
    function clearHyperactiveAtPositions(cardState, positions) {
        if (!cardState.specialStones || !cardState.specialStones.length) return;
    ...
    function getCardContext(cardState) {
        // Build context from unified specialStones for core logic compatibility
        const protectedStones = cardState.specialStones.filter(s => s.type === 'PROTECTED');

        // PERMA_PROTECTED, DRAGON, and BREEDING stones are immune to flipping
        const permaProtectedStones = cardState.specialStones.filter(s =>
            s.type === 'PERMA_PROTECTED' || s.type === 'DRAGON' || s.type === 'BREEDING' || s.type === 'ULTIMATE_DESTROY_GOD'
        ).map(s => ({
            row: s.row,
            col: s.col,
    ...
    function emitPresentationEvent(cardState, ev) {
        if (!cardState) return;
        // Prefer BoardOps central emitter which fills action meta (actionId, turnIndex, plyIndex)
        if (BoardOpsModule && typeof BoardOpsModule.emitPresentationEvent === 'function') {
            BoardOpsModule.emitPresentationEvent(cardState, ev);
            return;
        }
        if (!cardState.presentationEvents) cardState.presentationEvents = [];
        // Best-effort fallback: include minimal turnIndex and preserve provided fields
        const out = Object.assign({}, ev, { actionId: ev.actionId !== undefined ? ev.actionId : null, turnIndex: (cardState.turnIndex || 0), plyIndex: ev.plyIndex !== undefined ? ev.plyIndex : null });
        cardState.presentationEvents.push(out);
        // Also persist for UI fallback
        ...
        try { if (typeof globalThis !== 'undefined' && typeof globalThis.emitBoardUpdate === 'function') globalThis.emitBoardUpdate(); } catch (e) { }
    }
