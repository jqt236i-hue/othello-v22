...
(function (root, factory) {
    ...
    function processUltimateDestroyGodEffects(cardState, gameState, playerKey, deps = {}) {
        ...
        const udgs = (cardState.specialStones || []).filter(s => s.type === 'ULTIMATE_DESTROY_GOD' && s.owner === playerKey);
        ...
        for (const udg of udgs) {
            ...
            const before = (udg.remainingOwnerTurns === undefined || udg.remainingOwnerTurns === null)
                ? 0
                : udg.remainingOwnerTurns;
            const afterDec = before - 1;
            udg.remainingOwnerTurns = afterDec;
        ...
        if (cardState.specialStones) {
            cardState.specialStones = cardState.specialStones.filter(s =>
                s.type !== 'ULTIMATE_DESTROY_GOD' || (s.remainingOwnerTurns !== undefined && s.remainingOwnerTurns !== null && s.remainingOwnerTurns >= 0)
            );
        }
    ...
    function processUltimateDestroyGodEffectsAtAnchor(cardState, gameState, playerKey, row, col, deps = {}) {
        ...
        const udg = (cardState.specialStones || []).find(s =>
            s.type === 'ULTIMATE_DESTROY_GOD' && s.owner === playerKey && s.row === row && s.col === col
        );
        ...
        if (shouldDecrement) {
            ...
            udg.remainingOwnerTurns = before;
        }
