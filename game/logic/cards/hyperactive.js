...
(function (root, factory) {
    ...
    function moveHyperactiveOnce(cardState, gameState, entry, prng, deps = {}) {
        ...
        const clearHyperactiveAtPositions = deps.clearHyperactiveAtPositions || ((cs, positions) => { if (cs.specialStones) cs.specialStones = cs.specialStones.filter(s => !(s.type === 'HYPERACTIVE' && positions.some(p => p.row === s.row && p.col === s.col))); });
    ...
    function processHyperactiveMoves(cardState, gameState, prng, deps = {}) {
        ...
        const entries = (cardState.specialStones || [])
            .filter(s => s.type === 'HYPERACTIVE')
            .slice()
            .sort((a, b) => (a.createdSeq || 0) - (b.createdSeq || 0));

        for (const entry of entries) {
            if (!(cardState.specialStones || []).includes(entry)) continue;
    ...
    function processHyperactiveMoveAtAnchor(cardState, gameState, playerKey, row, col, prng, deps = {}) {
        const entry = (cardState.specialStones || []).find(s => s.type === 'HYPERACTIVE' && s.owner === playerKey && s.row === row && s.col === col);
