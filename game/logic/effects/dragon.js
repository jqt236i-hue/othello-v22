...
(function (root, factory) {
    ...
    function processDragonEffects(cardState, gameState, playerKey, deps = {}) {
        ...
        for (const dragon of dragons) {
            ...
            if (gameState.board[dragon.row][dragon.col] !== player) {
                dragon.remainingOwnerTurns = -1;
                continue;
            }

            // Turn countdown: decrement first, then fire if >= 0 (0 fires too)
            const before = (dragon.remainingOwnerTurns === undefined || dragon.remainingOwnerTurns === null)
                ? 0
                : dragon.remainingOwnerTurns;
            const afterDec = before - 1;
            dragon.remainingOwnerTurns = afterDec;
        ...
        if (cardState.specialStones) {
            cardState.specialStones = cardState.specialStones.filter(s =>
                s.type !== 'DRAGON' || (s.remainingOwnerTurns !== undefined && s.remainingOwnerTurns !== null && s.remainingOwnerTurns >= 0)
            );
        }

        if (converted.length > 0 && cardState.specialStones) {
    ...
    function processDragonEffectsAtAnchor(cardState, gameState, playerKey, row, col, deps = {}) {
        ...
        const dragon = (cardState.specialStones || []).find(s =>
            s.type === 'DRAGON' && s.owner === playerKey && s.row === row && s.col === col
        );
