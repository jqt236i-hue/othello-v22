...
(function (root, factory) {
    ...
    function applySwapWithEnemy(cardState, gameState, playerKey, row, col, deps = {}) {
        ...
        if ((cardState.bombs || []).some(b => b.row === row && b.col === col)) return result;

        // Check protection
        const isProtected = (cardState.specialStones || []).some(s =>
            s.row === row && s.col === col &&
            (s.type === 'PROTECTED' || s.type === 'PERMA_PROTECTED' || s.type === 'DRAGON' || s.type === 'BREEDING' || s.type === 'ULTIMATE_DESTROY_GOD')
        );
        ...
        } else if (cardState.specialStones && cardState.specialStones.length) {
            cardState.specialStones = cardState.specialStones.filter(s =>
                !(s.type === 'HYPERACTIVE' && s.row === row && s.col === col)
            );
        }
