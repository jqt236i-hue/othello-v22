...
function renderCardUI() {
    ...
    if (handBlackEl) {
        ...
        cardState.hands.black.forEach((cardId, idx) => {
            ...
            const usable = (isBlackTurn || isDebugHvH) && canInteract && hasNotUsedThisTurn && canAfford;

            // Add clickable class if it's black's turn and can interact (or debug HvH mode)
            if ((isBlackTurn || isDebugHvH) && canInteract) {
    ...
    // Render opponent's hand (hidden cards, no interaction)
    if (handWhiteEl) {
