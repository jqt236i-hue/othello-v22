...
const pattern = /(document\.|querySelector\(|createElement\()/g;
let found = false;
const matches = [];
walk(root, (p) => {
...
if (found) {
  console.error('\nERROR: DOM API usage detected under game/ (this should be in ui/).');
  matches.forEach(m => console.error(`${m.file}:${m.line}    ${m.text}`));
  process.exit(2);
}

console.log('OK: No DOM API usage detected under game/');
process.exit(0);
