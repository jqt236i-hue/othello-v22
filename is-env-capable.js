...
(function(root, factory) {
    ...
    function isDebugLogAvailable() {
        ...
        if (isBrowserEnv()) {
            return typeof window.debugLog === 'function';
        } else if (isNodeEnv()) {
    ...
    function safeDebugLog(message, level, meta) {
        if (isDebugLogAvailable()) {
            if (isBrowserEnv()) {
                window.debugLog(message, level || 'debug', meta || null);
            } else if (isNodeEnv()) {
    ...
    function safeGetDebugLogs(limit, levelFilter) {
        if (isBrowserEnv() && typeof window.getDebugLogs === 'function') {
            return window.getDebugLogs(limit, levelFilter);
